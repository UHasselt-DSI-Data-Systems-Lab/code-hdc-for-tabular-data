        ...
        for column, value, condition in selection_criteria:
            ...
            if value in self.config.cells:
                ...
                if self.model is not None:
                    ...
                    val_vector = self.gen_random_vectors(1)

                # add new value to config and item memory
                _ = self.config.add_value(value)
                self.item_memory = torch.cat([self.item_memory, val_vector], dim=0)  # type: ignore
                self.val_index.add(val_vector)  # type: ignore
...
class BSCDatabase(HDCDatabase):
    def __init__(self, dim: int, config: ExperimentConfig, name: str):
        """
        A class to represent a binary vector HDC database.
        Rows are encoded as key-value structures.
    ...
    def norm_hamming_sim(self, vec, vectors):
        """Calculate the normalized Hamming similarity between two binary hypervectors."""
        # Raw Hamming distance (count of differing bits)
        # Using torchhd.bind (XOR)
        diff = torchhd.bind(vec, vectors)
        hamming_distance = torch.sum(diff.to(torch.int32), dim=-1)

        # Normalization formula: 1 - (2 * D / d)
        # Result: 1.0 (identical), 0.0 (orthogonal), -1.0 (complement)
        norm_sim = 1 - (2 * hamming_distance.to(torch.float32) / self.dim)
        return norm_sim.cpu().numpy()

    def bind(self, vec1, vec2):
        """Bind two hypervectors together using circular convolution."""
        return vec1.bind(vec2)

    def bundle(self, vector_list):
        """Bundle a list of hypervectors together by summing and normalizing."""
        return torchhd.multibundle(vector_list)
    ...
    def gen_random_vectors(self, num_vectors):
        """Generate random HRR hypervectors for the given number of vectors."""
        return torchhd.BSCTensor.random(num_vectors, self.dim)

    def create_index(self, vectors):
        """Create a FAISS index for the given vectors."""
        index = faiss.IndexBinaryFlat(self.dim * 8)
        index.add(vectors.cpu().numpy().astype(np.uint8))  # type: ignore
        return index

    def get_item_memory(self):
        """Generate the item memory for the database (random only)"""
        return self.gen_random_vectors(self.config.item_memory_size)
    ...
    def encode_row(self, row_tuple):
        ...
        return self.bundle(torch.stack(bounds_list))
    ...
    def selection_vector(self, selection_criteria):
        ...
        for column, value, condition in selection_criteria:
            ...
            if value in self.config.cells:
                ...
                print(
                    f"Value '{value}' not found in item memory. Generating random vector."
                )
                val_vector = self.gen_random_vectors(1)
                ...
                _ = self.config.add_value(value)
                self.item_memory = torch.cat([self.item_memory, val_vector], dim=0)  # type: ignore
                self.val_index.add(val_vector.cpu().numpy().astype(np.uint8))  # type: ignore

            if condition == "!=":
                bundle_list.append(self.bind(col_vector, self.complement(val_vector)))
            else:
                bundle_list.append(self.bind(col_vector, val_vector))
        vector = self.bundle(torch.stack(bundle_list))
        return vector
    ...
    def select_rows(self, selection_criteria, threshold=0.5, top_k=0):
        ...
        vector = self.selection_vector(selection_criteria)
        if top_k > 0:
            D, K = self.row_index.search(
                np.array([vector.cpu().numpy().astype(np.uint8)]), top_k
            )  # type: ignore
            J = K[0]
        else:
            sim = self.norm_hamming_sim(vector, self.row_vectors)
            J = np.where(sim >= threshold)
        return self.rows[J]  # type: ignore

    def project(self, row_id, column):
        vector = self.row_vectors[row_id - 1]
        col_vector = self.item_memory[self.config.columns[column]]
        estimation_vector = self.bind(vector, col_vector)

        _, indexes = self.val_index.search(
            np.array([estimation_vector.cpu().numpy().astype(np.uint8)]), 1
        )  # type: ignore
        return self.config.cells_r[indexes[0][0] + self.config.col_count]
...
if __name__ == "__main__":
    ...
    hdc = BSCDatabase(dim=1000, config=config, name="random model")
    r = hdc.select_rows(
        [
            ("actor_1", "cch_pounder", "="),
